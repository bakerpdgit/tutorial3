...
class HashTable:
  ...
  def lookup(self, key: str) -> Any:
    ...
    if (item := self.__array[address]) is not None:
      ...
      if isinstance(
              item, tuple) and _______ == item[0]:  # No collision occurred
        return _______[1]
  ...
  def insert(self, key: str, value: Any) -> None:  # Insert method
    ...
    if self.__array[address] is None:  # Empty slot (no collision)
      ...
      if isinstance(self.__array[address],
                    tuple):  # If linked list not yet created
        node = self.__array[_________]
        self._________[address] = LinkedListNode(
            node, None)  # Create the head of the linked list
