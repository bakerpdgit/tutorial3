...
class Rectangle:
  ...
  def perimeter(self) -> float:  # a polymorphic method
    return 2 * (self.__length + ________)
  ...
  # we must first check if the object is a rectangle, then we check if their
  # attributes are the same in any order
...
class Circle:
  ...
  def area(self) -> float:  # a polymorphic method
    return pi * _______ ** ___

  def perimeter(self) -> float:  # a polymorphic method
    return 2 * pi * ________
